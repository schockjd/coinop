...
int gpio_set_value (unsigned int gpio, unsigned int value) {
  ...
  gpio_fd = open_gpio(gpio, O_WRONLY);
...
int gpio_get_value (unsigned int gpio, unsigned int *value) {
  ...
  gpio_fd = open_gpio(gpio, O_RDONLY);
...
int gpio_poll (unsigned int gpio, unsigned int lvl, unsigned int timeout) {
  ...
  syslog(LOG_INFO, "poll - %d asserted %s(%d) timeout %d", gpio, lvl?"high":"low", lvl, timeout);

  memset((void*)fdset, 0, sizeof(fdset));
  
  fdset[0].fd = open_gpio(gpio, O_RDONLY | O_NONBLOCK);
  ...
  do {
    ...
    syslog(LOG_INFO, "interrupt - gpio %d", gpio);  
    ...
    if (!trigger) {
      syslog(LOG_NOTICE, "spurious interrupt - gpio %d", gpio);
    }
  } while (!trigger);
  close(fdset[0].fd);
...
int open_gpio(unsigned int gpio, int flags) {
  ...
  syslog(LOG_INFO, "opening gpio %d", gpio);
